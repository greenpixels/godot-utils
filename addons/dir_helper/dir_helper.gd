...
static func for_each_in_directory(
	directory: String, callback: Callable, max_depth: int = 1
) -> void:
	...
	if dir:
		...
		while file_name != "":
			...
			if dir.current_is_dir() and max_depth > 0:
				for_each_in_directory(file_path, callback, max_depth - 1)
